...
ws.on("close", () => {
  ...
  setTimeout(() => {
    ...
    ws.on("open", () => { /* same as above */ });
    ws.on("message", (data) => { /* same as above */ });
    ws.on("error", (error) => { /* same as above */ });
    ws.on("close", () => { /* same as above */ });
  }, 5000);
});
...
// /poll – 4 separate polls with real-time Kraken prices
